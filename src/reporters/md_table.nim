...
method finalize(self: MdTableCheckFmt, stats: ReportStats) {.locks: "unknown".} =
  ...
  strm.writeLine(fmt"| Compliance | {stats.ratings.compliance.percent}%<br/> ![Badge - Compliance]({stats.ratings.compliance.badgeUrl}) |")
  strm.writeLine(fmt"| Openness | {stats.ratings.openness.percent}%<br/> ![Badge - Compliance]({stats.ratings.openness.badgeUrl}) |")
  strm.writeLine(fmt"| is hardware (factor) | {round(stats.ratings.hardware.factor)} |")
  strm.writeLine(fmt"| Quality | {stats.ratings.quality.percent}%<br/> ![Badge - OSH Quality]({stats.ratings.quality.badgeUrl}) |")
  strm.writeLine(fmt"| Machine-Readability | {stats.ratings.machineReadability.percent}%<br/> ![Badge - OSH Machine-Readability]({stats.ratings.machineReadability.badgeUrl}) |")
  strm.writeLine("")
  strm.writeLine("</details>")
